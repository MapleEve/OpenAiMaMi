# 1.2.4 后端 delta 6dims — relay::translator + anthropic_reasoning + anthropic_history + native_responses_compat + models

- baseline: 1.2.3 / target: 1.2.4 (macos-arm64 主，windows-x64 交叉锚定)
- 数据来源：只读 IDA 已落盘 `.c` 伪代码，不连 IDA、不重逆二进制。
- 模块范围：`core/relay/translator`、`core/relay/translator/stream`、`core/relay/anthropic_reasoning`(NEW)、`core/relay/anthropic_history`(NEW)、`core/relay/native_responses_compat`、`core/relay/models`。
- 标注约定：`[FULL decompile]`=完整可信；`[TRUNCATED]`/`[DECOMPILE-FAILED]` 不臆断（本次未命中，大文件均带 `[FULL decompile]` 头）。

---

## 0. 一句话结论（人话）

1.2.4 把 Anthropic relay 翻译层拆成「**推理决策**（anthropic_reasoning，新模块）+ **历史修正**（anthropic_history，新模块）+ **流式直转 Responses**（translator/stream 整体重写）」三段。核心新增能力是 **adaptive thinking**：按模型名（fable-5 / mythos-5 / sonnet-5 / opus 系等）和 codex effort 等级动态决定要不要给 Anthropic 请求塞 thinking budget；并在历史回放时 **SHA-256 校验/剔除非法 thinking signature**，防止上游拒绝。stream 侧把 1.2.3 的 `close_text_block/close_thinking_block/close_tool_blocks/flush` 一锅端，重写成统一的 `close_anthropic_block` 分发器 + `close_all_blocks` 迭代器 + `finish_response/fail_with_type` 收尾，新增 `redacted_thinking`、`reasoning_summary_*`、`custom_tool_call`、`tool_search_call` 等 Responses 事件类型。这些都是**中转代理内部翻译逻辑**，不动 IPC 名、不动持久化结构、不拉起/重启 Codex 进程。
...
### 3.3 translator 辅助字段
- `leading_think_prefix_decision` @0x10080a950：trim_start 后判 `<think>`(0x6968743C + 0x3E6B6E69 = "<think")前缀 → 返回 1=有前缀 / 2=无前缀 / 0=空串。
- `strip_leading_think_open_tag` @0x100809440：剥掉开头的 `<think>` 并 trim，无标签则返回 `0x8000000000000000`(None) 哨兵串。

---

## 4. error_paths 维度（逐条具体到变体）
...
## 5. persistence 维度（文件/DB/Keychain/config.toml）
...
- win delta `0x1408a2a20`(`uses_adaptive_thinking`) 与 mac `0x1005731d0` **逻辑等价**：9 个模型名明文串在 win 侧以 `aFable5/aMythos5/aMythosPreview/aSonnet5/aOpus5/aOpus48/aOpus47/aOpus46/aSonnet46` 形式直接命名（mac 侧对应 anon_279..286+"sonnet-4-6"），逐一对应。
- win `get_claude_web_search_compat`(@0x140156ef0) / `set_claude_web_search_compat`(@0x140e01580) / `set_codex_no_account_slots`(@0x140e2de30) 命令名串与 mac 侧对应 command 同名（IPC 契约跨平台一致）。
- 未发现 mac/win 行为分叉（无 `cfg(windows)` 专支）。

---

## 8. 函数清单（逐函数 name + behavior + destructive）

> destructive 列：列出该函数对入参/全局的可观测副作用（写内存、删字段、置标志）。本批无文件/进程破坏性。

| 函数 | 地址 | behavior | destructive |
|---|---|---|---|
| `normalize_model_name` | 0x100572da0 | trim+小写化+`.`/`_`→`-`，返回新 String | 分配返回串；原入参不改 |
| `uses_adaptive_thinking` | 0x1005731d0 | 模型名含 9 个新模型之一→true | 释放临时 normalized 串 |
| `adaptive_thinking_is_default` | 0x1005739f0 | fable/mythos/mythos-preview/sonnet-5→true，余 false | 同上 |
| `thinking_cannot_be_disabled` | 0x100573940 | fable-5/mythos-5→true | 同上 |
| `codex_effort_to_anthropic` | 0x100573340 | effort 串→anthropic effort 静态串/null | 释放临时小写串 |
| `effort_to_thinking_budget` | 0x100573640 | effort 等级→是否配 thinking budget(bool) | 释放临时小写串 |
| `reasoning_explicitly_disabled` | 0x100573ad0 | effort==off/none/disabled→true | 释放临时小写串 |
| `push_anthropic_message_block` | 0x100805ec0 | 追加 assistant/user 消息块；末条同 role 则合并 content 数组 | **写** messages Vec（grow/insert）；drop 入参 Value |
| `push_anthropic_thinking_block` | 0x10080a9b0 | thinking 块插到末条 assistant 的 content 数组（跳过已有 thinking/redacted_thinking）| **写** content 数组（insert_mut，含 memmove）|
| `push_tool_result_block`(history) | 0x1004af550 | tool_result 块合并到末条 user 消息的 content（跳过已有 tool_result）| **写** content 数组 |
| `leading_think_prefix_decision` | 0x10080a950 | 判 `<think>` 前缀→0/1/2 | 无 |
| `strip_leading_think_open_tag` | 0x100809440 | 剥 `<think>` 头+trim，无则 None | 分配返回串 |
| `is_anthropic_relay_private_item` | 0x10080c0a0 | 判 item 是否 relay 私有（thinking/redacted_thinking/tool_use/tool_result），否则委托 `is_openai_private_response_item` | 无（只读）|
| `collect_loaded_custom_tool_names` | 0x10080d800 | 递归收集 custom tool 名字(name 字段)入 HashMap | **写** 入参 HashMap(insert) |
| `collect_tool_search_output_tools` | 0x10080dae0 | 递归收集 tool_search 输出工具入 Vec | **写** 入参 Vec(extend)；含 `unwrap_failed` 路径(0x10080deb3) |
| `unwrap_custom_tool_input` | 0x1007f6380 | 把 custom tool 的 input(JSON 串) 解包：若 JSON object 含 `input`(5) 键取其值，否则回退原串；带 `web_search`/`patch`/`system` 分支 | 分配返回串；drop 中间 serde Value/Error |
| `anthropic_to_responses_response_with_tool_maps` | 0x100829870 | **非流式**一次性翻译：把整个 Anthropic response(JSON Value) 同步转成 Responses response 对象，收 tool_map+tool_name_map 双图。产出 output 项含 `custom_tool_call`(16)/`function_call`(13)/`encrypted_content`(17)/reasoning；stop_reason 映射 status：命中 `content_filter`(7)/`max_output_tokens`(10)/len29 SSE 串 → `incomplete`+对应 stop，否则 `completed`+`summary_text`。复用 finish_response 同款信封(id/object:"response"/created_at/status/model/output/usage) | **读**入参 Anthropic Value；**构造**返回 Responses BTreeMap；[FULL decompile 3192 行]；多处 serde_json `unwrap_failed` |
| `AnthropicDirectToResponsesStream::with_tool_maps` | 0x100acf9f0 | 构造 stream（uuid+timestamp+双 tool map）| **写** self 全字段 |
| `::close_all_blocks` | 0x100ad0f40 | 迭代 blocks BTreeMap，逐个 close_anthropic_block，drop 剩余 | **清空** self@+25..+27 blocks 根 |
| `::close_anthropic_block` | 0x100ad10d0 | 按 block type 分发收尾，发 content_part.done/output_item.done/reasoning_summary_*.done/output_text.done 事件，处理 redacted_thinking | **写** event sink；[FULL decompile] |
| `::close_anthropic_tool_block` | 0x100ad52a0 | tool 块收尾：发 custom_tool_call_input.done/function_call_arguments.done/output_item.done；状态 completed/incomplete | **写** event sink；[FULL decompile] |
| `::finish_response` | 0x100acfdc0 | 终结响应：发 response.completed/incomplete+usage | **置** finished@253=1；**写** event sink |
| `::fail_with_type` | 0x100aced60 | 失败终结：发 response.failed+error | **置** finished@253=1；**写** event sink |
| `normalize_messages` | 0x1004ac900 | 历史消息规范化（含 SHA-256 常量 0x6A09E667/0x5BE0CD19/0x9B05688C/0xA54FF53A，疑 thinking signature 计算/校验）| **改写** messages 数组；[FULL decompile] |
| `rectify_invalid_thinking_signature` | 0x1004afd70 | 剥离非法 thinking signature | **删** thinking/redacted_thinking 块(return 0)；**删** object 块的 `signature` 字段；retain 消息 |
| `rectify_invalid_thinking_signature::{closure}` | 0x1009af720 | retain_mut 闭包：thinking/redacted 块 **删除并计数**(return 0)，object 块删 signature 后保留(return 1) | **删** thinking/redacted 块、signature 字段；**写** 3 个移除计数器 |
| `trailing_turn_supports_thinking` | 0x1004afa50 | 末轮能否挂 thinking（tool_result 引用闭环判定）| 释放临时 Vec/HashSet |
| `strip_hosted_web_search_tools` | 0x1003f5f60 | 剥离 hosted web_search 工具+tool_choice | **删** tools 数组项/键、tool_choice 键；返回剥离计数 |
| `contains_http_status_code` | 0x10057e240 | 串是否含 3 位 HTTP 状态码 | 无（只读）；越界 panic_bounds_check |
| `models::serialize`(×3 单态) | 0x10057cbe0/cd30/cea0 | CodexNoAccountSlot → JSON | **写** serializer 输出 |
