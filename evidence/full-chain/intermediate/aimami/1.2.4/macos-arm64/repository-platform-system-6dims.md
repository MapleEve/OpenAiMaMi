...
| `set_claude_web_search_compat` | mac+win 双平台对齐（mac `0x10079bbc0` 完整 setter / win `0x140e01580`） | win `0x140e01580` | NEW | 写 enabled 标志，带状态机轮询（case 0/1/2/3）+ Arc 引用计数clone，返回 `CoreEnvelope`；win 串 `aSetClaudeWebSe`(28)/`aManager_1`(7)/`aEnabled_8`(7) |
| `set_codex_no_account_slots` | mac+win 双平台对齐（mac `0x10079a8a0` 完整 setter / win `0x140e2de30`） | win `0x140e2de30` | NEW | 设置 codex 无账号槽位数 + `relaunch` 布尔；win 串 `aSetCodexNoAcco`(26)/`aManager_1`(7)/`aSlots`(5)/`aRelaunch`(8) |

补充：mac `commands::relay::serialize @0x1007b6530` 是 `CodexNoAccountSlotsPayload` 的 serde 序列化实现（DTO），证明该 DTO 在 mac 同名存在；set 命令的完整 setter 实现体（`RelayManager::set_codex_no_account_slots @0x10079a8a0`，724 行）已确认存在于 `core::relay::manager`，与本 DTO 序列化非同一函数，早期误判为「仅 DTO」已据盘上 .c 更正（详见下文「跨平台一致性备注」）。
...
- `set_codex_no_account_slots`：更正（原合并条目声称"mac delta 仅含 DTO serialize"为陈旧误判，两个 setter 均不成立）——mac 侧已有完整 setter 实现，见 raw/aimami/1.2.4/macos-arm64/core/relay/manager/ida/pseudocode/set_codex_no_account_slots_0x10079a8a0.c（724 行完整 body，符号 `RelayManager::set_codex_no_account_slots @ 0x10079a8a0`，slots/relaunch payload 反序列化 + RelayState 写回 + persist，无 kill/spawn/exit），与 win 侧 `set_codex_no_account_slots@0x140e2de30` 行为对等，非仅 DTO serialize。mac@0x10079a8a0 / win@0x140e2de30 双平台对齐，无需补 mac 等价路径。
- `set_claude_web_search_compat`：更正（原合并条目声称"mac delta 仅含 DTO serialize"为陈旧误判，两个 setter 均不成立）——mac 侧已有完整 setter 实现，见 raw/aimami/1.2.4/macos-arm64/core/relay/manager/ida/pseudocode/set_claude_web_search_compat_0x10079bbc0.c（156 行，符号 `RelayManager::set_claude_web_search_compat @ 0x10079bbc0`，双互斥锁 + panic-poison 检查 + RelayState 比对 + 条件 persist + sanitize_for_export 返回），与 win 侧 `set_claude_web_search_compat@0x140e01580` 行为对等，非仅 DTO serialize。mac@0x10079bbc0 / win@0x140e01580 双平台对齐，无需补 mac 等价路径。
