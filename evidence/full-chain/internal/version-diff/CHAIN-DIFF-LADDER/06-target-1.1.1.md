...
**2026-06-22** · 公开清洗版 · 只发布摘要、计数、gate 姿态和仓库相对材料入口
...
| macOS delta | 43 条 delta；43/43 readyToImplement；44 same；39 orphan-no-backend；10 frontend removed |
| Windows delta | 60 条 gate-final delta；`get_image_compat` 读向删除不进有效分母；59/59 readyToImplement；26 same；41 orphan-no-backend；10 frontend removed |
...
### B2. Windows frontend removed 命令（10 条）
...
| relay/system | `set_relay_provider_network`、`test_relay_draft`、`test_relay_provider`、`reset_codex_config` | 被新 stream、router/switch 或 restart 链路吸收。 |

Windows custom-instruction 三条在 1.1.1 中按 orphan-no-backend 分类，属于无 backend 的产品边界，不计入 frontend removed；`get_image_compat` 是 Windows 读向 command removed，但作为 delta 分母外 removed 单独处理。
...
| gate | 计数 | 命令 |
|---|---:|---|
| readyToImplement | 59/59 | Windows effective delta denominator 全部命令 |
| strictImplementationUse | 0 | 无 |
| lower | 0 | 无 |

Windows gate-final 原始 delta 为 60 条；`get_image_compat` 在 Windows 1.1.1 中属于读向 command removed，不计入有效分母。最终 rollup 取 59 条有效分母，59/59 readyToImplement；旧漂移口径已被 2026-06-20 win-100 终账取代。

### B5. Windows accounting 与后端完整性

| 项 | 计数 | 说明 |
|---|---:|---|
| delta | 59 | 行为/DTO/副作用真变化，均已 readyToImplement |
| same | 26 | 逐命令 body 等价确认 |
| orphan-no-backend | 41 | 无 Windows backend 的前端命令面，按产品边界处理 |
| frontend removed | 10 | plugin / passthrough / relay test / reset config 等前端删除项 |
| backend truly full | true | 四角度后端完整性已收口 |
| truly undone | 0 | 无剩余待逆缺口 |

公开仓只保留上述清洗后的 accounting 与 gate 姿态，不发布 raw producer 证据、机器路径、反编译正文或 IDB sidecar。
...
- Windows custom-instruction 三条按 orphan-no-backend 处理，不作为 frontend removed 或可实现 backend。
- 1.1.1 后端完整性四角度已闭合，公开口径为 `backend_truly_full=true`、`truly_undone=0`。
