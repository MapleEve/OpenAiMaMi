...
| `set_claude_web_search_compat` | mac+win 均锚定完整 setter（本轮第3轮更正，见下方说明） | mac `0x10079bbc0` / win `0x140e01580` | NEW | 写 enabled 标志，带状态机轮询（case 0/1/2/3）+ Arc 引用计数 clone，返回 `CoreEnvelope`；win 串 `aSetClaudeWebSe`(28)/`aManager_1`(7)/`aEnabled_8`(7) |
| `set_codex_no_account_slots` | mac+win 均锚定完整 setter（本轮第3轮更正，见下方说明） | mac `0x10079a8a0` / win `0x140e2de30` | NEW | 设置 codex 无账号槽位数 + `relaunch` 布尔；win 串 `aSetCodexNoAcco`(26)/`aManager_1`(7)/`aSlots`(5)/`aRelaunch`(8) |

补充：mac `commands::relay::serialize @0x1007b6530` 是 `CodexNoAccountSlotsPayload` 的 serde 序列化实现，证明该 DTO 在 mac 同名存在。**本轮(第3轮整合复查)更正**：~~原陈述"set 命令实现体不在本批 delta 制品里"~~为陈旧误判——mac 侧完整 setter 实现体已确认存在于 `raw/aimami/1.2.4/macos-arm64/core/relay/manager/ida/pseudocode/set_claude_web_search_compat_0x10079bbc0.c`（157 行）与 `set_codex_no_account_slots_0x10079a8a0.c`（725 行），均非截断，逻辑与 win 侧对应 setter 等价（双互斥锁+panic-poison 检查+RelayState 比对+条件 persist）。
...
- `set_claude_web_search_compat` / `set_codex_no_account_slots`：win delta 锚定了完整 setter 实现。~~原陈述"mac delta 仅含对应 DTO 的 serialize，setter 实现体不在本批 NEW-delta"~~ — **本轮(第3轮整合复查)更正**：为陈旧误判，mac 侧两函数均已确认存在完整 setter 实现（`set_claude_web_search_compat_0x10079bbc0.c` 157 行 / `set_codex_no_account_slots_0x10079a8a0.c` 725 行，均非截断），逻辑与 win 侧对应 setter 等价。
