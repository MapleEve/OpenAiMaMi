...
> 生产者:mini 本机。双 IDA 已加载 1.2.3(mac <反编译器>✓ / win <反编译器>✓)。全 sonnet worker + Workflow 并行 + orchestrator 持授权亲逆 IDA。
...
- ✅ **11 超大截断函数全逆全**:MCP decompile 有响应上限截成桩;改用 <工具调用> 直调 `ida_<反编译器>.decompile` + 7KB 分页取回完整伪代码(40-92KB 各),覆盖截断桩(红线13 不许以体积 bail)。mac 0 残留 TRUNCATED / 11 标 `[FULL <反编译器>]`。
