...
本轮为 reduce/distillation pass，**未持有 IDA MCP 工具作为常驻装备**（无 `mcp__ida-pro-mcp-*` 前缀工具），改为通过既有脚本 `idacall.py`（JSON-RPC 一次性客户端，位于 `<临时路径> 等多个临时目录，为既有 pass 遗留工具，非本轮新增）对 `<内网IP>:13337/mcp` 发起只读验证调用。红线 17 <门控> 已核：
...
"auto_analysis_ready":true,"<反编译器>_ready":true,"strings_cache_ready":true}
```
