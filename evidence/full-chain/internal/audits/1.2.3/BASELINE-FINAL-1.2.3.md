# BASELINE-FINAL — AiMaMi 1.2.3 完整基线(独立完整,非 delta)

> 1.2.3 已成为新 baseline。本文件是 1.2.3 独立完整基线交付(全 1638 后端函数 + 6 前端页真逆),非版本 delta。
> 所有数字经 orchestrator 扫盘 grounded 复核(截断守卫 红线20 + 名↔体一致校验)。

## 1. 后端完整基线(mac,1638 全 App 函数)

### 覆盖(grounded)
- **1638 全 App 函数 decompile**:1422 baseline same-set + 216 delta(88 超大体用 IDA 反编译器分页全解,红线13;1 IDA 失败已用反汇编兜底)。名↔地址一致(用 IDA 函数枚举地址,非符号 VA)。
...
| relay-translator | 192 | translator(Anthropic↔OpenAI Chat↔Responses 双向转换)/stream(4 个 SSE 流状态机)/models/codex_catalog/image_compat(视觉模型剥图)/quota(多 provider 余额)/storage(原子写+MiniMax M3 迁移)/atomic_write/keychain(三级密钥)/dialects(reasoning 注入) |
...
## 3. IDB 命名(红线24,双平台)
- **mac 1.2.3**:1638 App 函数归入 94 目录 + 全注释 + idb_save。
- **win 1.2.3**(strip):938 模块归属 + 115 命令 handler 精确命名 + 242 跨平台字符串签名真名 + 15 delta 锚点 + idb_save。

## 4. 与 delta 的区别
- delta(`COVERAGE-FINAL-1.2.3.md`)= 只逆相对 1.2.2 新增 216。
- **本基线** = 1638 全后端 + 6 页全前端独立完整逆向,不依赖 1.2.2 迁移假设,不用命令 diff 代替前端。1.2.3 现为 baseline。
