# DELTA-LOGIC-1.2.4 — macOS relay-catalog-thread-diag 相对 1.2.3 的行为差异

来源：`raw/aimami/1.2.4/macos-arm64/version-delta/relay-catalog-thread-diag-6dims.md`（6dims 详实分析，19177B）+ `raw/aimami/1.2.4/macos-arm64/version-delta/DELTA-FINAL-1.2.4.md`（跨平台整合终稿，27345B）。
baseline：1.2.3（`macos-1.2.3-relay`，638 去重函数，consumerStartReady 23/23）。
本批范围：5 个 `core::relay` 子模块的 45 个 1.2.4 NEW-delta 函数，全部 `[FULL decompile]`，0 截断桩。

## 1. 一句话结论

1.2.4 在 relay 层新增了一条「无账号模型槽（no-account slot）路由 + 线程 provider 收养（orphan adoption）+ 诊断修复」流水线，落点是 `codex_router_catalog.json`（路由目录）、`config.toml`（config_takeover 重写）、`threads` 表（SQLite UPDATE）和一组 journal/recovery 文件；同时引入硬编码的 `gpt-5.x` 系列 native 模型 slug 与 `bundled`/`matching-account-cache` 目录源枚举。
...
> mac setter 主体 `RelayManager::set_codex_no_account_slots@0x10079a8a0` 在 manager/ 模块，出本批 5 模块范围；本批覆盖 setter 调用的库层。
...
> 残留：`NoAccountModelRestoreEntry` 的精确字段集由 journal 语义 + restore SQL 反推（`model`/`model_provider`/`rollout_path` 等），serialize impl 用 serde derive，逐字段名未在伪代码中以明文 4 字节键全量出现。行为已闭合，仅字节级键枚举不完整。
...
### Keychain
- 本批 delta 伪代码内 **未** 出现 Keychain/security-framework 调用；凭据写入不在此模块范围（在 router_unlock_auth bundle 范围）。

## 7. sidecar（Dim 6 — 进程 / Codex 重启）
...
## 8. 逐函数清单（name + behavior + destructive）
...
## 9. Win parity（windows-x64 delta，14 锚点确认跨平台一致）
...
- Keychain：本批伪代码内未观测到，不在此模块声明（在 router_unlock_auth bundle 范围）。
