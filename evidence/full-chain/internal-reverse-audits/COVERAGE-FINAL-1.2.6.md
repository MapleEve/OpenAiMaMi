...
| 函数 | macOS 地址 | Windows 地址 | 双端状态 |
|---|---|---|---|
| `reconcile_inner`（`codex_config_reconciler::reconcile_inner`） | `0x1002E6840` | `0x140AAB170` | ✅ 双端 confirmed+加注释；win 侧已改名+归目录+`idb_save` |
| `config_takeover::takeover` | `0x100AECDE0` | `0x140574EE0` | ✅ 双端 confirmed+加注释；win 侧已改名+归目录+`idb_save` |
| `router_unlock_auth::commit_auth_for_router` | `0x1007325B0` | `0x140BF4CE0` | ✅ 双端 confirmed+加注释；win 侧已改名+归目录+`idb_save` |
| `codex_project_state::repair_if_needed` | `0x100737150`（仅确认地址，本体未展开） | `0x1405D7DD0`（1456B） | ✅ **v6 本轮 confirmed**——round3/round4 的排除系假墙误判，round5 用字符串特征法破墙（mac侧内嵌错误模板字符串精确反查+39字节成功字面量逐字节核对+调用图双重顶层入口+panic-Location两跳独立确认，三条证据链交叉印证），已改名+归目录+加注释+`idb_save`；**独立对抗核验裁定真实confirmed，可放行** |
...
- **（本次更新新增，v5 round3）** 下一层递归函数 4 项中 3 项本轮 confirmed：`reconcile_inner`@`0x140AAB170`（此前 v3 仅为未获自身锚点的尾调用候选）、`config_takeover::takeover`@`0x140574EE0`、`commit_auth_for_router`@`0x140BF4CE0`，win 侧均已改名+归目录+`idb_save`。本轮同批清理这 3 个函数在 `/vibe/`（rename 工具默认落点）下的残留 dirtree 条目，BFS 复验各自仅剩 1 个正确模块路径下的条目（0 stranded）。
...
- **工具层遗留问题（非逆向证据缺口）**：MCP `decompile()` 工具对 win `0x1406FABE0` 的渲染/缓存层 bug（本轮独立第三次复现）本身未修复，需工具维护者介入排查 MCP 服务端实现；IDB 底层数据（`func_cmt`/`line_cmt`）已核实正确，消费方若需查看该函数的准确身份结论，应通过 `py_eval` 直查 `idc.get_func_cmt()` 而非 MCP `decompile()` 工具的默认渲染。
...
**（本次更新新增，round3）识别/清理侧确认为架构性天花板，非"仍未找到"**：round3 对宿主函数 `0x140A42B20` 做全量完整反编译（332303 字符，0 截断，用 py_eval 切片法绕过 MCP 预览截断），并纠正 round2 对 2 个下游 helper 的角色误判——`0x140B9FFD0` 实际是 base64 解码驱动（`vendor_base64_decode_driver`），`0x14086E7A0` 实际是 JSON 解析+尾随字符校验（`vendor_json_parse_str_with_trailing_check`），round2 记录的角色描述与实际完全调换，本轮已订正改名。**核心结论**：逐行 control-flow 追踪 + 全二进制字段名 xref（`encrypted_content` 字段名数据地址全二进制仅 2 处代码引用：写入侧+本读取侧，无第三处）确认——**Windows 侧对私有前缀的清理逻辑架构上是"先解码重建、失败即丢弃"（判定与重建融合在同一段代码里，无独立前置 predicate），而 macOS 侧 `is_unusable_store_false_reasoning_item` 是"先纯判定、从不尝试解码"的独立 predicate 函数**。两者最终处理效果相同（不可用条目均不出现在输出中），但代码结构完全不同——**这是穷尽核查后confirm的真实架构差异（`genuine_architectural_difference_confirmed=true`），不是"还没找到"的真实缺口**。
...
- 已做：claim-first 账本登记（开工前核 `REVERSE-STATUS.md`/`INDEX.jsonl`/该目录无碰撞）、owner-gate 撞车检查、IDA rename/dirtree/`idb_save`（写入侧函数+2 调用者+2 helper）、`INDEX.jsonl`/`REVERSE-STATUS.md` 追加（本轮 round2/round3 对应 claim_id 均已独立核实存在）。
...
> session `cc-consumer-reduce-win126-nonvoice49-20260815`；消费者面向收口（flow.md 步骤 7），无 IDA/无 raw 重写，不提升任何实现门。

- **总账**：49 non-voice 目标 = **41 DONE**（dim1-5 闭合，dim6 留白）+ **8 BLOCKED**（blocks_start 已记录）。voice active count=**0**（硬门满足）。
- **门禁分层**：41 DONE 目标 leaf 级 `consumerStartReady=true` + `strictImplementationUse=true`；`readyToImplement=0`、`implementation_use=false`、`gate_accepted=false`（dim6 留白 + 8 BLOCKED → 最高档不闭合）。8 BLOCKED 目标 `consumerStartReady=false`。
- **自足消费者合同**：`audits/windows-1.2.6-version-delta/logic/CONSUMER-CONTRACT-NONVOICE-49.md`（消费者不翻 raw 即可读 DTO/success/error/副作用/Unknown 分类）。
- **8 BLOCKED**：angle_b:clean(BLOCKED_DIAGNOSTIC_ONLY) / get_claude_web_search_compat / set_claude_web_search_compat / get_codex_router_auth_readiness / remove_mcp_server / set_codex_no_account_slots + angle_c:run_bundled_cli / set_anthropic_thinking_carrier。
- 非动作：不写产品仓、不公开发布、不做真机对照（红线 25）、不覆盖删除历史内容。

<!-- HANDLER_RESOLVED_WIN126_8BLOCKED_2026-08-16 -->
### 8 原 BLOCKED 目标 → handler RESOLVED + DTO 字段级闭合（2026-08-16 追加）

> session `cc-consumer-reduce-win126-nonvoice49-handler-resolved-20260816`；消费侧归约收口（非新 IDA 生产），承接 round4/round5/deep-recovery 深挖结果，不提升任何实现门。

- **8 原 BLOCKED 目标的 handler 身份静态 RESOLVED + dim4 字段级闭合**（0 残留 BLOCKED）：`set_claude_web_search_compat` handler=`sub_1409B6420`（写 +318 bool）+ `sub_1409C0B10`(persist)；`set_codex_no_account_slots` handler=`sub_140DC9E00`(validate)+`sub_140C774F0`(字段写+persist+transition)；`get_claude_web_search_compat`=`0x14014ade0` 读 +318；`get_codex_router_auth_readiness`=`0x140547c70` 复合位域；`remove_mcp_server`=`0x140156D90`→`0x1409F42E0`→`0x1409D0ED0`；`clean`=`0x140AF0C50`(semantic_recovery)；`run_bundled_cli`=`0x140555b60`(CONFIRMED)；`set_anthropic_thinking_carrier`=`0x140a28fb0`(100% parity)。
- **DTO 字段级闭合**：`ClaudeWebSearchCompatPayload={enabled}`；`CodexNoAccountSlotsPayload={state,codexLaunchError,warnings}`；`CodexNoAccountSlot={providerId,model,catalogSlug}`；**`McpServerSummary` 9 字段**；**`CleanResult` 3 字段**（authBackupsRemoved/registryBackupsRemoved/staleEntriesRemoved）；`CoreEnvelope={schemaVersion,success,code,message,payload}`。
- **vtable 填充枚举**：`off_141889D80`（8 槽，set_codex_no_account_slots）、`off_14188A280`（7 槽，set_claude_web_search_compat）——破除「动态派发 handler 不可恢复」假上限。
- **前端 CCF 闭合**：get_claude_web_search_compat 读 `response.data.enabled`；remove_mcp_server hover 删除→确认框→mutate；clean 维护页按钮→`toast maintenance.cleanResult(...)`。
- **门禁（不变）**：leaf 级证据档闭合 only；bundle 级仍 BLOCKED_N0（load_snapshot decompile_null + dim6 + live_reference）。`consumerStartReady`/`strictImplementationUse`/`readyToImplement`/`implementation_use`/`gate_accepted` 均不变。自足合同见 `audits/windows-1.2.6-version-delta/logic/CONSUMER-CONTRACT-NONVOICE-49.md` §2。

<!-- CONSUMER_CONVERGENCE_MAC126_NONVOICE13_2026-08-16 -->
## macOS 1.2.6 non-voice 13 目标消费者收口 + 非命令后端 truly_undone 归零（2026-08-16 追加）

> session `cc-consumer-reduce-mac126-nonvoice13-20260815`（13 目标合同）+ `cc-mac126-truly-undone-fullchain-20260816`（2 truly_undone 收口）；消费者面向收口终稿，不提升任何实现门。

- **13 non-voice delta 目标**：7 added + 6 removed/renamed，dim1/2/3/5 闭合（delta-fullchain-leaf-20260809）+ dim4 body 级逐字段闭合（2026-08-15）。1 BLOCKED（`get_codex_router_auth_readiness`，diagnostic-only-live-reference-required）。voice active count=0（硬门）。
- **非命令后端 angle-D 四分类（2026-08-16）**：1639 非命令后端函数 = 183 already_in_canonical + 66 glue/local_outtake + 2 fullchain + 其余 baseline 符号迁移。`four_categories_rollup_251` 落盘核验 `truly_undone 2→0`、`already_in_canonical 183→185`、`deep_leaf_1_2_6 3→5`。
- **2 真缺口 fullchain 收口（truly_undone 归零为真零）**：`is_relay_protocol_object@0x1009fb2b0`（core::relay::translator，纯谓词 memcmp+SIMD 判定 relay 协议 key 集，无副作用）+ `migrate_legacy_auth_state@0x100733fe0`（core::relay::router_unlock_auth，legacy 虚拟认证态迁移，字段级 DTO/error 逐字节闭合，⚠ 写本机 auth 状态）。此前 `truly_undone=0` 是 BOUNDED 2-regex angle-C 假零（5 非命令后端簇 25 semantic leaf 真实实现）。自足合同见 `audits/macos-1.2.6-version-delta/logic/CONSUMER-CONTRACT-NONVOICE-13.md` §1.4。
- **门禁（不变）**：leaf 级 `consumerStartReady=true` + `strictImplementationUse=true`；bundle 级 `readyToImplement=false`、`implementation_use=false`、`gate_accepted=false`（dim6 留白 + LIVE_REFERENCE_NOT_RUN）。mac 与 win 互不外推（红线8）。

<!-- DUAL_PLATFORM_SOT_CLOSEOUT_20260816 -->
## 双平台 SoT 收口 + grounded 六项终验（2026-08-16 追加）

> session `cc-consumer-reduce-aimami126-dualplatform-sot-closeout-20260816`；消费侧归约收口终验（红线14），无 IDA/无 raw 重写，不提升任何实现门。

- **win**：8 原 BLOCKED 目标 handler RESOLVED（0 残留）+ DTO 字段级闭合（McpServerSummary 9 字段 / CleanResult 3 字段 / CoreEnvelope 错误信封）+ vtable 填充枚举 off_141889D80/off_14188A280。bundle 级仍 BLOCKED_N0（load_snapshot decompile_null + dim6 + live_reference）。
- **mac**：truly_undone 2→0 真零（is_relay_protocol_object 纯谓词 + migrate_legacy_auth_state 字段级 DTO/error 闭合）；非命令后端 1639 函数 angle-D 四分类闭合。leaf 级 consumerStartReady=true + strictImplementationUse=true。
- **grounded 六项终验全 PASS（红线14）**：① INDEX 尾 session-id；② root producer-ledger 存在；③ REVERSE-STATUS 1.2.6 零 in-progress；④ leaf 抽查（truly_undone 2 叶七件套 + win claude-web-search-compat 10 伪代码）；⑤ 双端 gate-report 非空；⑥ 双端 diff-readiness readiness=true。
- **实现门全 false**（dim6 留白 + live_reference 未对照，红线25）；mac 与 win 互不外推（红线8）。

<!-- CODEX_DIAGNOSTIC_BTIER_9TARGET_DUALPLATFORM_20260821 -->
## codex_diagnostic B-tier 9-target dual-platform batch（2026-08-21 追加）

> parent claim session `cc-aimami126-codex-diagnostic-btier-claim-20260821`；macOS reduce `cc-aimami126-codex-diagnostic-btier-reduce-mac-20260821`；windows reduce `cc-aimami126-codex-diagnostic-btier-reduce-win-20260821`；消费者面向收口（flow.md 步骤 7），本轮为新 raw + 双平台首次归约（非既有 delta 迁移）。

- **范围**：`relay/codex_diagnostic`（macOS 记为 `core::relay::codex_diagnostic`，windows 记为 `codex_diagnostic`）9 个 diagnostic key targets：`project_state_integrity_diagnostic_key_verify` / `router_http_contract` / `state_db_relocation` / `takeover_backup_orphan` / `thread_model_catalog_consistency` / `thread_response_item_ids` / `thread_poisoned_instructions` / `thread_provider_consistency_with_cache` / `requires_quiet_window`，× {macos-arm64, windows-x64}。
- **macOS 结果**：8/9 leaves `consumerStartReady=true`（`requires_quiet_window` 证据不完整未评级——仅1个owner，缺evidence.md/manifest.json/gate-report.json/interface-report.json/call-tree.json）。全部9 leaves `strictImplementationUse=false`（dim1 前端 Tauri invoke 入口本轮全模块未定位，无 pre-1.2.6 baseline 可迁移）。纠正1 leaf（`thread_poisoned_instructions`）自报的 `strictImplementationUse=true` 为 `false`；提升1 leaf（`thread_provider_consistency_with_cache`）自报的 `no_gate_promotion` 为 `consumerStartReady=true`。
- **windows 结果**：全部 9/9 leaves `consumerStartReady=false`（`gate-report.json` status=`BLOCKED_DIM1_UNIVERSALLY_MISSING`），因为 windows reduce 判定 dim1/`uiCallEntryReady` 缺失会阻断 `consumerStartReady` 本身，而非仅阻断 `strictImplementationUse`。8/9 leaves 定位到 owner（`thread_provider_consistency_with_cache` 未定位 owner，判 `gap_needs_reducer`，已排除假墙 taxonomy，非 `accepted_unknown`）。
- **⚠ 未裁决的跨平台 gate 冲突（本轮 SoT 核实新发现，非既有裁决翻案）**：同一条"dim1/前端本轮全模块未调查"的缺失证据，macOS reduce 判定其只阻断 `strictImplementationUse`（`consumerStartReady` 仍可 true），windows reduce 判定其连 `consumerStartReady` 本身也阻断。两个同日 reduce session 对 GATE-SPEC.md Gate 1 `uiCallEntryReady` 必要项的适用判断互相矛盾。本轮 SoT 核实未擅自选边，已在 `REVERSE-STATUS.md`（本 claim closure 条目）与 `INDEX.jsonl`（windows audit_bundle 条目 note）逐处标记，留待用户裁决。
- **⚠ windows INDEX.jsonl 追加缺口，本轮核实发现并回填**：windows reduce 自身 `data/producer-ledger.json` 声称 9 个 raw leaf 的各自 producer session 已完成 INDEX 追加（"46 hits"），reduce 最终对用户的六项磁盘核查也声称 PASS（含具体行数 2042→2051）——两者均经独立 `grep`/`wc -l` 核验为假（核实时刻均为 0 命中/行数不符）。本轮已补写 9 条 raw_leaf 条目 + 1 条 audit_bundle 条目（`INDEX.jsonl` 428→438 行，仅追加），每条 `status` 字段原样记录该差异，不代为掩盖。
- **门禁（不变，本轮未新增实现门写入）**：`readyToImplement=false`、`implementation_use=false`、`gate_accepted=false`（两平台全部 target）。dim6 留白（C5 实现侧职责）；live reference parity 未对照（红线25）。mac 与 win 互不外推（红线8，windows 的全阻断判定不影响 macOS 8/9 已给出的 `consumerStartReady=true`，反之亦然——留待用户对两者哪个更贴合 GATE-SPEC.md 原意做裁决，不由本轮替代裁决）。
- 产物：`audits/macos-1.2.6-codex_diagnostic/`（README.md/manifest.json/gate-report.json/logic/FULL-CHAIN-codex_diagnostic.md/pointers/evidence-paths.md/data/producer-ledger.json）+ `audits/windows-1.2.6-codex_diagnostic/`（README.md/manifest.json/gate-report.json/logic/DIAGNOSTIC-KEYS-1.2.6-WIN.md/pointers/evidence-paths.md/reviews/CLAUDE.md/data/producer-ledger.json）。
...
- **consumer_gate 状态**：mac `consumerStartReady=true` + `strictImplementationUse=true`（leaf dim1-5 closed）；win `consumerStartReady=true` + `strictImplementationUse=false`（BLOCKED_N0: load_snapshot decompile_null + dim6 + live_reference）
