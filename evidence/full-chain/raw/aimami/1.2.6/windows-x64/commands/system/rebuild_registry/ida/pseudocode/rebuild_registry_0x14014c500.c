// Pseudocode for rebuild_registry (EA: 0x14014c500, size: 0x3d8)
// Module: commands/system
// Source: IDA Hex-Rays decompiler
// IDA Comment: Tauri 命令 rebuild_registry 处理器（commands::system）。重建本地注册索引（mac 符号 Repository::rebuild_registry_with_policy 佐证）。⚠副作用：重写注册数据。
// Tauri 命令 rebuild_registry 处理器（commands::system）。重建本地注册索引（mac 符号 Repository::rebuild_registry_with_policy 佐证）。⚠副作用：重写注册数据。
__int64 __fastcall rebuild_registry(__int64 a1)
{
  __int64 v2; // rax
  __int64 v3; // rdx
  __int64 v4; // rcx
