// Pseudocode for force_kill_codex (EA: 0x14014bf20, size: 0x3d8)
// Module: commands/system
// Source: IDA Hex-Rays decompiler
// IDA Comment: Tauri 命令 force_kill_codex 的 handler：强制终止 codex 进程（破坏性副作用：强杀进程），经 mutationGate。归属：命令名串精确 xref 唯一归属（live 复验）；模块按命令族（diagnose/restart_codex 锚定 src\commands\system.rs）推断。
// Tauri 命令 force_kill_codex 的 handler：强制终止 codex 进程（破坏性副作用：强杀进程），经 mutationGate。归属：命令名串精确 xref 唯一归属（live 复验）；模块按命令族（diagnose/restart_codex 锚定 src\commands\system.rs）推断。
__int64 __fastcall sub_14014BF20(__int64 a1)
{
  __int64 v2; // rax
  __int64 v3; // rdx
  __int64 v4; // rcx
