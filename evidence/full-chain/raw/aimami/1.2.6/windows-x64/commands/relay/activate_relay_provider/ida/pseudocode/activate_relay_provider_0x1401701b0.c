// Pseudocode for activate_relay_provider (EA: 0x1401701b0, size: 0x5bb)
// Module: commands/relay
// Source: IDA Hex-Rays decompiler
// IDA Comment: Tauri 命令 activate_relay_provider 的 handler：按入参 providerId 激活指定 relay provider，经 relay manager 切换当前活跃 provider 并持久化状态。归属：命令名串精确 xref 唯一归属（live 复验串 0x14177a9e8→本函数）；模块按命令族+同文件 panic-Location 锚定推断为 src\c
// Tauri 命令 activate_relay_provider 的 handler：按入参 providerId 激活指定 relay provider，经 relay manager 切换当前活跃 provider 并持久化状态。归属：命令名串精确 xref 唯一归属（live 复验串 0x14177a9e8→本函数）；模块按命令族+同文件 panic-Location 锚定推断为 src\commands\relay.rs。
__int64 __fastcall sub_1401701B0(__int64 a1)
{
  __int64 v2; // r14
  __int64 v3; // rdx
  __int64 v4; // rcx
  ...
  v32 = v17;
  v33 = &v41;
  sub_1404F7410(&v35, &v28);
  LOBYTE(v44) = v35.m256i_i8[0];
  if ( v35.m256i_i8[0] == -1 )
  {
    sub_1405471E0(&v25, v2, &v39);
