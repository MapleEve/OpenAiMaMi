// Pseudocode for deactivate_relay_provider (EA: 0x14016f920, size: 0x5bb)
// Module: commands/relay
// Source: IDA Hex-Rays decompiler
// IDA Comment: Tauri 命令 deactivate_relay_provider 的 handler：按入参 providerId 停用 relay provider，经 relay manager 清除活跃标记并持久化。归属：命令名串精确 xref 唯一归属（live 复验）；模块按命令族推断 src\commands\relay.rs。
// Tauri 命令 deactivate_relay_provider 的 handler：按入参 providerId 停用 relay provider，经 relay manager 清除活跃标记并持久化。归属：命令名串精确 xref 唯一归属（live 复验）；模块按命令族推断 src\commands\relay.rs。
__int64 __fastcall sub_14016F920(__int64 a1)
{
  __int64 v2; // r14
  __int64 v3; // rdx
  __int64 v4; // rcx
