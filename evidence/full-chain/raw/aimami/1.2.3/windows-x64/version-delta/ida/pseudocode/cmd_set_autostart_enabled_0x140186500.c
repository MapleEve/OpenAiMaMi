// win 1.2.3 delta | cmd_set_autostart_enabled @ 0x140186500 | changelog autostart
// [FULL IDA decompiler 5176B]
// win 1.2.1 | tauri command handler = set_autostart_enabled | mapped via command-name string xref (win-native, ground-truth)
__int64 __fastcall set_autostart_enabled(__int64 a1)
{
  __int64 v2; // r14
  __int64 v3; // r14
  __int128 v4; // rdi
  __int64 v5; // r15
  __int64 v6; // rbx
  __int64 v7; // rax
  __int64 v8; // rcx
  __int64 v9; // rsi
  _BYTE v11[512]; // [rsp+30h] [rbp-50h] BYREF
  __int64 v12; // [rsp+230h] [rbp+1B0h]
  _BYTE v13[384]; // [rsp+238h] [rbp+1B8h] BYREF
  __int64 v14; // [rsp+3B8h] [rbp+338h]
  int v15; // [rsp+3C0h] [rbp+340h]
  int v16; // [rsp+3C4h] [rbp+344h]
  _BYTE v17[152]; // [rsp+3C8h] [rbp+348h] BYREF
  const char *v18; // [rsp+460h] [rbp+3E0h] BYREF
  __int64 v19; // [rsp+468h] [rbp+3E8h]
  const char *v20; // [rsp+470h] [rbp+3F0h] BYREF
  __int64 v21; // [rsp+478h] [rbp+3F8h]
  _BYTE *v22; // [rsp+480h] [rbp+400h]
  __int128 *v23; // [rsp+488h] [rbp+408h]
  __int64 v24; // [rsp+490h] [rbp+410h]
  _BYTE v25[48]; // [rsp+5F0h] [rbp+570h] BYREF
  __int128 v26; // [rsp+620h] [rbp+5A0h]
  __int128 v27; // [rsp+630h] [rbp+5B0h]
  __int128 v28; // [rsp+690h] [rbp+610h] BYREF
  __int64 v29; // [rsp+6A0h] [rbp+620h]
  _OWORD v30[2]; // [rsp+6A8h] [rbp+628h] BYREF
  __int64 v31; // [rsp+6C8h] [rbp+648h]
  _BYTE v32[48]; // [rsp+6D0h] [rbp+650h] BYREF
  __int128 v33; // [rsp+700h] [rbp+680h]
  __int128 v34; // [rsp+710h] [rbp+690h]
  __int128 v35; // [rsp+720h] [rbp+6A0h] BYREF
  __int128 v36; // [rsp+730h] [rbp+6B0h]
  __int128 v37; // [rsp+740h] [rbp+6C0h] BYREF
  __int64 v38; // [rsp+750h] [rbp+6D0h]
  __int64 v39; // [rsp+760h] [rbp+6E0h]
  __int64 v40; // [rsp+768h] [rbp+6E8h]
  char v41; // [rsp+775h] [rbp+6F5h]
  char v42; // [rsp+776h] [rbp+6F6h]
  char v43; // [rsp+777h] [rbp+6F7h]
  __int64 v44; // [rsp+778h] [rbp+6F8h]

  v44 = -2;
  sub_141684120(v11, a1, 520);
  sub_141684120(v13, a1 + 520, 400);
  v38 = *(_QWORD *)(a1 + 936);
  v37 = *(_OWORD *)(a1 + 920);
  v24 = 0;
  v18 = aSetAutostartEn;
  v19 = 21;
  v20 = aApp;
  v21 = 3;
  v22 = v11;
  v23 = &v37;
  v43 = 1;
  sub_1401C3650(v25, &v18);
  if ( *(_DWORD *)v25 == -1 )
  {
    *(_OWORD *)&v32[24] = *(_OWORD *)&v25[24];
    *(_OWORD *)&v32[8] = *(_OWORD *)&v25[8];
    sub_141684120(&v18, a1 + 520, 360);
    v35 = *(_OWORD *)(a1 + 880);
    *(_QWORD *)&v36 = *(_QWORD *)(a1 + 896);
    *(_QWORD *)v32 = 1;
    v43 = 0;
    sub_14047E370((unsigned int)&v18, v14, (unsigned int)v32, (unsigned int)&v35, v15, v16);
    goto LABEL_22;
  }
  sub_141684120(v17, v25, 152);
  v24 = 0;
  v18 = aSetAutostartEn;
  v19 = 21;
  v20 = aRepo;
  v21 = 4;
  v22 = v11;
  v23 = &v37;
  v2 = sub_140003640((volatile void *)(v12 + 16));
  if ( !v2 )
  {
    sub_1416850A0(v30, &v18, &v20);
    if ( LOBYTE(v30[0]) != 0xFF )
    {
      *(_OWORD *)&v32[24] = v30[1];
      *(_OWORD *)&v32[8] = v30[0];
      sub_141684120(&v18, a1 + 520, 360);
      v35 = *(_OWORD *)(a1 + 880);
      *(_QWORD *)&v36 = *(_QWORD *)(a1 + 896);
      *(_QWORD *)v32 = 1;
      v42 = 0;
      sub_14047E370((unsigned int)&v18, v14, (unsigned int)v32, (unsigned int)&v35, v15, v16);
      v43 = 0;
      sub_140014150(v17);
      goto LABEL_22;
    }
    v2 = *((_QWORD *)&v30[0] + 1);
  }
  v24 = 0;
  v18 = aSetAutostartEn;
  v19 = 21;
  v20 = aEnabled;
  v21 = 7;
  v22 = v11;
  v23 = &v37;
  v42 = 1;
  sub_1401BCE80(&v35, &v18);
  LOBYTE(v40) = v35;
  if ( (_BYTE)v35 == 0xFF )
  {
    sub_140487EA0(v32, v17, v2, BYTE1(v35));
    sub_141684120(&v18, a1 + 520, 400);
    if ( *(_QWORD *)v32 != -1 )
    {
      v27 = v34;
      v26 = v33;
      *(_OWORD *)&v25[32] = *(_OWORD *)&v32[32];
      *(_OWORD *)&v25[16] = *(_OWORD *)&v32[16];
      *(_OWORD *)v25 = *(_OWORD *)v32;
LABEL_21:
      v41 = 0;
      sub_1404384C0(&v18, v25);
      goto LABEL_22;
    }
    v3 = *(_QWORD *)&v32[8];
    *((_QWORD *)&v4 + 1) = *(_QWORD *)&v32[24];
    v5 = *(_QWORD *)&v32[16];
    if ( *(__int64 *)&v32[24] < 0 )
    {
      v6 = 0;
      goto LABEL_13;
    }
    if ( *(_QWORD *)&v32[24] )
    {
      nullsub_1();
      v6 = 1;
      v7 = sub_140001650(*((_QWORD *)&v4 + 1), 1);
      if ( !v7 )
      {
LABEL_13:
        v39 = v5;
        v40 = v3;
        sub_1416C2D4B(v6, *((_QWORD *)&v4 + 1));
      }
      *(_QWORD *)&v4 = v7;
      sub_141684120(v7, v5, *((_QWORD *)&v4 + 1));
    }
    else
    {
      *(_QWORD *)&v4 = 1;
    }
    if ( v3 )
      sub_140001660(v5, v3, 1);
    v25[8] = 3;
    *(_QWORD *)&v25[16] = *((_QWORD *)&v4 + 1);
    *(_OWORD *)&v25[24] = v4;
    *(_QWORD *)v25 = -1;
    goto LABEL_21;
  }
  *(_OWORD *)&v32[24] = v36;
  *(_OWORD *)&v32[8] = v35;
  sub_141684120(&v18, a1 + 520, 360);
  v28 = *(_OWORD *)(a1 + 880);
  v29 = *(_QWORD *)(a1 + 896);
  *(_QWORD *)v32 = 1;
  sub_14047E370((unsigned int)&v18, v14, (unsigned int)v32, (unsigned int)&v28, v15, v16);
  v43 = 0;
  sub_140014150(v17);
LABEL_22:
  if ( (_QWORD)v37 != -1 )
  {
    v8 = *((_QWORD *)&v37 + 1);
    v39 = v38;
    v40 = 0;
    v31 = *((_QWORD *)&v37 + 1);
    while ( v39 != v40 )
    {
      ++v40;
      v9 = v8 + 96;
      sub_1402C7520();
      v8 = v9;
    }
    if ( (_QWORD)v37 )
      sub_140001660(*((_QWORD *)&v37 + 1), 96 * v37, 8);
  }
  return sub_140014270(v11);
}
