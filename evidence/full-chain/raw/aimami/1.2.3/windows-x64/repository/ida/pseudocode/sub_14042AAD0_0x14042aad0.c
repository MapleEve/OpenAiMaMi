// module: codexmate_lib/core/repository
// addr: 0x14042aad0
// name: sub_14042AAD0
// win 1.2.1 | module src/core/repository.rs | attributed via call-graph propagation (>=2 same-module callers) (win-native)
char __fastcall sub_14042AAD0(__int64 *a1, __int64 a2)
{
  unsigned __int64 v4; // rax
  __int64 v6; // rbx
  __int64 v7; // r14
  __m128i v8; // xmm0
  __m128i v9; // xmm6
  __int64 v10; // rsi
  __int64 v11; // rdi
  __int64 i; // r15
  unsigned __int64 v13; // rax
  __m128i v14; // xmm8
  unsigned __int64 v16; // r13
  char result; // al
  int v18; // eax
  bool v19; // zf

  if ( !a1[3] )
    return 0;
  v4 = sub_140378810(a1 + 4, a2);
  _RCX = v4 >> 57;
  v6 = *a1;
  v7 = a1[1];
  v8 = _mm_cvtsi32_si128(v4 >> 57);
  v9 = _mm_shuffle_epi32(_mm_shufflelo_epi16(_mm_unpacklo_epi8(v8, v8), 0), 68);
  v10 = *(_QWORD *)(a2 + 8);
  v11 = *(_QWORD *)(a2 + 16);
  for ( i = 0; ; i += 16 )
  {
    v13 = v7 & v4;
    v14 = _mm_loadu_si128((const __m128i *)(v6 + v13));
    _R12D = _mm_movemask_epi8(_mm_cmpeq_epi8(v14, v9));
    if ( _R12D )
      break;
LABEL_7:
    _RCX = (unsigned int)_mm_movemask_epi8(_mm_cmpeq_epi8(v14, (__m128i)-1LL));
    if ( (_DWORD)_RCX )
      return 0;
    v4 = i + v13 + 16;
  }
  while ( 1 )
  {
    __asm { tzcnt   ecx, r12d }
    v16 = v13;
    _RCX = -(__int64)(v7 & (v13 + _RCX));
    if ( v11 == *(_QWORD *)(v6 + 24 * _RCX - 8) )
    {
      _RCX = (unsigned int)sub_1416847B0(v10, *(_QWORD *)(v6 + 24 * _RCX - 16), v11);
      result = 1;
      if ( !(_DWORD)_RCX )
        return result;
    }
    v18 = _R12D - 1;
    LOWORD(v18) = _R12D & (_R12D - 1);
    v19 = (_WORD)v18 == 0;
    _R12D = v18;
    v13 = v16;
    if ( v19 )
      goto LABEL_7;
  }
}