...
3. **Windows-platform backend**: Windows 1.0.9 gate-report records `accepted_substitute` as "Current 内部构建 custom-instructions chain is live product/local behavior; upstream 1.0.9 backend owner parity is absent substitute." This does not confirm Windows 1.1.1 presence — Windows delta is tracked separately.
