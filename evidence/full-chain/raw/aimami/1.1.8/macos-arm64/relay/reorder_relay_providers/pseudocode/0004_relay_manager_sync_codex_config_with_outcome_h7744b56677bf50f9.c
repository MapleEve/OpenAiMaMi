// addr=0x1005c8130
// depth 2 — re-snapshots state, computes valid_codex_active_ids, and reconciles the on-disk Codex router config with the outcome of the reorder/persist call (cross-subsystem side effect: mutates codex-side routing config, not just relay's own file).
