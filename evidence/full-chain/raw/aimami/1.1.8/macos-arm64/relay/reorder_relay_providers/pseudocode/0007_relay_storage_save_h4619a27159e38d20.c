// addr=0x1004b2680
// depth 3 (child of persist) — ensures the config directory exists (CodexPaths::ensure_directories), serde-serializes the top-level RelayState as a JSON map with fields schemaVersion/providers/activeByIde/proxy/codexRouterEnabled/codexRouterNoAccountMode (confirmed via get_string on the serialize_entry key-length arguments), then calls atomic_write::write_atomic.
