// addr=0x10039e600
// depth 4 — genuine atomic-write-with-rename persistence primitive: Path::parent -> DirBuilder::_create(dir) -> builds a process-id+uuid-v4 temp filename -> OpenOptions::_open -> Write::write_all -> File::sync_data -> std::sys::fs::rename (the commit point) -> sync_parent_dir; std::sys::fs::remove_file used only on the cleanup/rollback path. terminated_reason=persistence_commit at std::sys::fs::rename (depth 5 from owner).
