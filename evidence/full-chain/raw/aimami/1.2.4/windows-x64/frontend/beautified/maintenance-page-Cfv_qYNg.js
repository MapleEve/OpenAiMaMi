...
function $e({
  ...
  const {
    t: s
  } = z(), [i, u] = r.useState(null), [g, v] = r.useState(!1), [C, N] = r.useState(null), [D, E] = r.useState(!1), [_, f] = r.useState(null), [R, x] = r.useState(null), A = g || D || C !== null, j = async (d = !1) => {
    d ? v(!0) : (U.flushSync(() => v(!0)), await O()), f(null), x(null);
    const c = Date.now();
    try {
...
function Ge({
  ...
  const {
    t: u
  } = z(), [g, v] = r.useState(!1), C = e.status === "ok" ? Q : e.status === "warning" ? fe : Ue, N = e.status === "ok" ? "text-emerald-500" : e.status === "warning" ? "text-amber-500" : "text-red-500";
  return a.jsxs("div", {
...
function ia() {
  const {
    t: e
  } = z(), l = ye(), [s, i] = r.useState({}), [u, g] = r.useState({}), [v, C] = r.useState(!1), [N, D] = r.useState(!1), [E, _] = r.useState(!1), [f, R] = r.useState(""), x = Y({
    ...
    _(!1), w("debugUpload", () => L.mutateAsync(f.trim()))
  }, ue = [{
    key: "diagnose",
    icon: ze,
