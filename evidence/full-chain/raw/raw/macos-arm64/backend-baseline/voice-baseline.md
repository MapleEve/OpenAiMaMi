...
**截断/反编译失败检查**：对全部 182 个文件头做 `TRUNCATED` / `DECOMPILE-FAILED` 关键字扫描，**0 命中**——本批文件全部为完整反编译产物，无需标注截断函数。其中 2 个巨型函数（`aliyun_asr/connect` 下的 `hbb4b340bd79fba8f_0x100285c80.c` / `_0x10045f2b0.c`，各约 90937B / 92782B 反编译体）文件头明确标注 `[FULL — IDA decompiler 全解 XXXXXB, 超大体分页取回]`，即：体积巨大但确认是**完整**反编译产物，只是取回时分页拉取，不属于截断。
