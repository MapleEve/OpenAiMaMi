...
| 后端 | 已把原开源能力和公开证据可支撑部分纳入六边形架构骨架、DTO/contract owner 和可替换文件边界。 | 闭源业务不做全量还原；无公开证据行为只保留边界、接口、DTO、错误语义、测试占位、待处理状态、空操作或可替换骨架。 | `npm run validate:backend`、`npm run validate:backend-cargo` |
