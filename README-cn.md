...
| 前端 | 入口、Provider、路由、运行时、服务门面、缓存、i18n、E2E mock、模块 owner、app-shell index query owner、accounts/sessions 当前源码链路、custom-instructions 当前源码链路、重建证据索引和部分收口验证已按公开证据归位。 | 不在 README 声明双平台、全叶子节点或全部文案已经 100% 完成；没有证据、收口记录和验证器支撑的内容不写成已完成。 | `npm run validate:frontend` |
| 后端 | 六边形目录、命令适配、用例层、核心层、仓储层、平台层、适配器、合同、DTO、公开文件事实聚合、账号监视仓储预检、通知客户端状态 settings 持久化、skills 文件事务 owner、会话仓储写入状态与 effect/status 边界已进入聚合验证。 | 不做闭源业务全量还原；无公开证据支撑的业务保持类型化待实现、空操作或可替换骨架边界；不声明动态插件执行、真实平台副作用、外部进程、市场安装、真实网络重试、完整服务启动或 `voice` 集成已经完成。 | `npm run validate:backend` |
