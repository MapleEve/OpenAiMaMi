...
- 已把账号切换、账号删除和登出事务从 `repository/accounts.rs` 上移到 `application/usecase/accounts.rs`：usecase 负责读取 registry、校验目标账号、校验 snapshot、备份 auth、复制 snapshot、删除 snapshot、删除 auth、标记或清空 active、保存 registry 和组装 `SwitchPayload` / `RemovePayload` / `LogoutPayload`；repository 只暴露 registry 读写、snapshot 路径、auth 备份、auth 删除和 snapshot 删除等窄文件边界，账号 registry 领域模型已移动到 `core/model/accounts.rs`。
- `scripts/validate-backend-accounts-owner.mjs` 与 `validate:backend-accounts-owner` 已扩展为账号事务 owner 门禁，用于防止 `repository/accounts.rs` 重新暴露 `switch_account`、`remove_accounts` 和 `logout` 用户动作事务，并校验这些编排仍在 application/usecase 内通过窄 repository helper 完成。
...
- Accounts 已完成 `switch_account` / `switch_account_and_restart_codex`、`remove_accounts` 和 `logout` 的 usecase owner 迁移；`export_accounts_to_file`、`preview_account_import` 和 `import_accounts_from_file` 仍需要继续从 repository-heavy 事务迁到 application/usecase。
