...
| 前端 | 入口、Provider、路由、运行时、服务门面、缓存、i18n、E2E mock、模块 owner 和部分当前源码链路已有收口记录与验证入口。 | 仍以全量还原为目标；不声明双平台、全叶子节点或全部文案已经 100% 完成，除非公开证据、收口记录和验证脚本都能支撑。 | `npm run validate:frontend` |
| 后端 | 六边形目录、命令适配、用例层、核心层、仓储层、平台层、适配器、合同、DTO 和部分公开证据支撑能力已进入聚合验证。 | 不做闭源业务全量还原；无公开证据支撑的业务保持类型化待实现、空操作或可替换骨架边界。 | `npm run validate:backend` |
