...
README 只保留长期有效的归纳状态，不按提交、日期或临时补丁追加流水账。具体模块进度、证据细节和逐项收口记录放在 `docs/reconstruction/source-map.md`、对应重建文档和验证脚本中。
...
| 后端 | 六边形目录、命令适配、用例层、核心层、仓储层、平台层、适配器、合同、DTO、skills 文件事务归属和部分公开证据支撑能力已进入聚合验证。 | 不做闭源业务全量还原；无公开证据支撑的业务保持类型化待实现、空操作或可替换骨架边界。 | `npm run validate:backend` |
