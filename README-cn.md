...
- **前端还原基础**：`src/` 已承载公开前端源码，并按入口、路由、全局 Provider、运行期初始化、模块 owner、缓存 owner、i18n 和 E2E mock 建立可继续还原的主流模块化结构；完整 leaf 仍按 raw/internal 证据继续补齐。
- **后端公开范围**：`src-tauri/` 已建立 commands、application、core、platform、repository、adapters、contracts 六边形骨架，并补回已有公开证据支撑的后端能力；系统类、平台类、诊断类和热点类能力按独立 owner 收口，未公开业务不写成真实逻辑。
- **验收入口**：公开边界、README 同步、前端证据、前端分层、状态合同、可见文案、后端六边形、后端 owner、合同 owner、i18n 和 E2E mock 已有脚本化检查入口，可用于判断当前源码是否仍守住公开范围。
...
- **提交口径**：每次提交都必须同步更新 README 当前状态；更新只能归纳状态变化，不写成按提交逐条追加的流水账。
