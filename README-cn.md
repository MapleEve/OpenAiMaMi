...
| 文件事实能力 | Sessions 已补已扫描文件删除，并从 rollout/session JSONL 读取更新时间、项目路径、父会话、agent 字段和 turn 计数；Analytics 已补公开会话文件事实聚合、平均 turn 映射和变更分析命令分类/日序列聚合；System 已补配置清理和更新可安装性只读检测。 |
...
| 后端闭环 | Relay、Daemon、Sessions、Analytics、MCP、System 等后端能力不声明真实网络、后台线程、SQLite threads 表、全局状态、真实 token/tool/quota 统计口径、server 启动、更新安装执行、诊断修复、外部进程重启或 shell 闭环。 |
