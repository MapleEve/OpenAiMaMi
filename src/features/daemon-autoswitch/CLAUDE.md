# daemon-autoswitch 前端模块 Claude 执行规则
...
## daemon-autoswitch 模块归属

- 本目录是 daemon-autoswitch 复杂模块的唯一 模块归属。
...
- 无 raw/internal 证据支撑的 daemon-autoswitch 行为只能保留边界类型、测试占位和待补证据位置。
