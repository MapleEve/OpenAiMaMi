...
| `src-tauri/src/core/model/analytics.rs` | 确认 usage/session/tool/change/quota 的公开事实聚合由 core model owning。 |
...
| pending 项 | 当前边界 |
| --- | --- |
| `load_token_analytics` | 只保留公开 IPC / DTO 骨架和 pending status；不声明闭源 token analytics、敏感字段路径、input/output/reasoning 真实统计口径已恢复。 |
...
| 运行时统计精确口径 | `active_minutes_estimate`、tool/change 分类和 quota history 只来自公开可重建文件事实；不声明真实运行时统计口径或闭源一致性。 |
...
- `scripts/validate-backend-analytics-owner.mjs` 必须验证本 evidence map 存在，且记录公开 session/rollout/quota-history 文件事实聚合、token analytics pending 边界、quota-history 7 天 compaction 和不声明闭源 token analytics。
