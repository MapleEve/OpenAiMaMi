...
| accounts transfer 后端文件事务 owner | `docs/reconstruction/accounts-transfer-current-source-map.md` | 账号导入导出的 usecase 事务、repository `json.tmp` 加 `rename` 原子替换、platform hostname owner 和未恢复闭源账号业务边界；由 `scripts/validate-backend-accounts-transfer-owner.mjs` 直接验证。 |
...
| settings 后端 current-source owner 边界 | `docs/reconstruction/settings-current-source-evidence-map.md` | settings command adapter、settings usecase、settings repository、settings core model、settings contracts、API proxy 平台端口、200ms TCP 探针、platform_actions 委托、Tauri 注册、package/后端聚合入口和未声明边界；由 `scripts/validate-backend-settings-owner.mjs` 与 `npm run validate:backend-settings-owner` 直接验证，不作为闭源 settings 业务恢复、业务 HTTP 探测完成、真实 update/restart 平台副作用、hotspot/mystery/notification/daemon 业务 owner 或 `voice` 接入声明。 |
| 后端合同 owner / DTO 边界 | `src-tauri/src/contracts/mod.rs`、`src-tauri/src/contracts/*.rs` | contracts 模块拆分、DTO re-export、默认响应边界和 voice 空骨架边界；由 `scripts/validate-backend-contract-owners.mjs` 与 `npm run validate:backend-contract-owners` 直接验证，不接入 `voice`，不声明闭源业务恢复。 |
...
| system snapshot-bootstrap 后端当前源码链路 | `docs/reconstruction/system-snapshot-bootstrap-current-source-map.md` | `load_snapshot`、`load_bootstrap_state`、Tauri 注册、command adapter、usecase owner、repository owner、Rust DTO、TypeScript service、IPC contract、E2E mock 和未声明边界；由 `scripts/validate-backend-system-owner.mjs` 直接验证，只把 `load_snapshot` 关闭到 snapshot-bootstrap 公开文件事实 owner，不声明 raw/internal gate、真实 daemon、watcher、autoswitch、maintenance/window 平台副作用、full_leaf、full_leaf_100 或 `voice` 接入。 |
...
| 前后端 relay owner 骨架 | `docs/reconstruction/relay-core-current-source-evidence-map.md` | 本地配置 repository 恢复、relay_thread_migration 空操作/待处理边界和 relay test retry 错误语义由 `scripts/validate-backend-relay-owner.mjs` 直接验证；不作为 raw/internal strict gate 关闭依据，不声明真实进程扫描、线程 patch、SQLite patch、session meta 替换、Codex 重启或运行时迁移已恢复。 |
...
| system hotspot 前端链路、macOS has_notch 只读探针与后端边界 | `docs/reconstruction/system-hotspot-current-source-map.md` | hotspot frontend chain、Rust hotspot owner、macOS `has_notch` 只读屏幕形态探针和 strict gate 未恢复边界。 |
...
| system-window-maintenance 前端当前源码链路 | `docs/reconstruction/system-window-maintenance-current-source-map.md` | 七条 non-gating closed command、`load_snapshot` snapshot-bootstrap owner、route/service/query/mutation/cache/mock/IPC contract 和 maintenance/window 平台副作用未声明边界；由 `scripts/validate-frontend-system-window-maintenance-current-source.mjs` 直接验证。 |
...
| 托盘 current-source owner 与公开文件事实读取边界 | `docs/reconstruction/tray-current-source-evidence-map.md` | Windows accepted tray target、current-source extra、前后端 tray owner、`tray_relay_usage_quota_model` 公开本地文件事实读取和未恢复 native tray / runtime relay 边界；由 `scripts/validate-backend-tray-owner.mjs` 与 `scripts/validate-frontend-tray-current-source.mjs` 直接验证。 |
...
| settings API proxy 当前源码链路 | `docs/reconstruction/settings-api-proxy-current-source-map.md` | 由 `scripts/validate-frontend-settings-api-proxy-current-source.mjs` 与 `npm run validate:frontend-settings-api-proxy-current-source` 专名验证 UI、controller、service、API、IPC、cache、mock 当前源码链路、mock env/probe payload 和后端受限平台端口边界；不声明 100% 还原、full leaf 完成、全功能完成、业务 HTTP 探测完成、系统代理扫描完成或 `voice` 接入。 |
