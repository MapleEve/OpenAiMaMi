...
| `docs/reconstruction/settings-current-source-evidence-map.md` | settings 后端 current-source owner 边界 | `scripts/validate-backend-settings-owner.mjs` 与 `npm run validate:backend-settings-owner` 直接验证本文、settings command adapter、settings usecase、settings repository、settings core model、settings contracts、platform_actions 委托、Tauri 注册、package/后端聚合入口和未声明边界；不作为闭源 settings 业务恢复、API proxy 真实网络探测、真实 update/restart 平台副作用、hotspot/mystery/notification/daemon 业务 owner 或 `voice` 接入声明。 |
...
| `docs/reconstruction/settings-api-proxy-current-source-map.md` | settings API proxy 当前源码链路 | `scripts/validate-frontend-settings-api-proxy-current-source.mjs` 与 `npm run validate:frontend-settings-api-proxy-current-source` 专名验证 settings API proxy 的 UI、controller、service、API、IPC、cache、mock 当前源码链路和未做边界；不声明 100% 还原、full leaf 完成、全功能完成、真实网络探测完成、系统代理扫描完成或 `voice` 接入。 |
