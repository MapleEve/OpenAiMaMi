...
| `hotspot.rs` | `has_notch`、`get_hotspot_enabled`、`set_hotspot_enabled`、`hotspot_ready`；其中 `has_notch` 只恢复 macOS 只读屏幕形态探针。 |
...
| `settings.rs` | `set_api_proxy_config`、`test_api_proxy_config`、`detect_api_proxy_config`、`get_usage_refresh_interval`、`set_usage_refresh_interval`、`check_update_installability`、`graceful_restart_for_update` |
| `system.rs` | `load_snapshot`、`refresh_usage_snapshot`、`load_bootstrap_state`、`clean`、`rebuild_registry`、`diagnose`、`set_auto_switch`、`configure_auto_switch`、`run_daemon_once`、`restart_codex`、`force_kill_codex`、`reset_codex_config`、`open_path`、`get_system_info`、`focus_main_window`、`get_device_id`、`get_notification_client_state`、`get_mystery_unlock_grants`、`merge_mystery_unlock_grants`、`get_or_create_remote_device_secret`、`import_remote_device_secret_if_empty`、`load_pending_auto_switch`、`dismiss_pending_auto_switch`、`confirm_pending_auto_switch`、`confirm_pending_auto_switch_and_restart_codex`、`get_image_compat`、`set_image_compat` |
...
| `docs/reconstruction/app-shell-source-only-current-source-map.md` | app-shell source-only 当前源码边界 | `scripts/validate-frontend-app-shell-source-only.mjs` 与 `npm run validate:frontend-app-shell-source-only` 直接验证 app-shell index asset、remote secret runtime、`desktop-message` source-only 查询、popover consumer、system service wrapper 与 `platform_actions` 公开边界；不提升 covered，不编造 endpoint，不声明 update/restart/window-path 后端完整恢复，也不接入 `voice`。 |
...
| `docs/reconstruction/accounts-transfer-current-source-map.md` | accounts transfer 后端文件事务 owner | `scripts/validate-backend-accounts-transfer-owner.mjs` 直接验证账号导入导出 usecase 事务、repository `json.tmp` 加 `rename` 原子替换、platform hostname owner 和未恢复闭源账号业务边界。 |
...
| `docs/reconstruction/settings-current-source-evidence-map.md` | settings 后端 current-source owner 边界 | `scripts/validate-backend-settings-owner.mjs` 与 `npm run validate:backend-settings-owner` 直接验证本文、settings command adapter、settings usecase、settings repository、settings core model、settings contracts、API proxy 平台端口、200ms TCP 探针、platform_actions 委托、Tauri 注册、package/后端聚合入口和未声明边界；不作为闭源 settings 业务恢复、业务 HTTP 探测完成、真实 update/restart 平台副作用、hotspot/mystery/notification/daemon 业务 owner 或 `voice` 接入声明。 |
| `docs/reconstruction/cross-home-usage-frontend-current-source-map.md` | cross-home-usage 前端当前源码链路 | `scripts/validate-frontend-cross-home-usage-current-source.mjs` 与 `scripts/validate-frontend-current-source-closeouts.mjs` 直接验证本文、首页 overview usage load/refresh、analytics usage panel、service/API 门面、mock、IPC contract、closeout 台账和未声明边界；不作为 raw/internal gate、后端逆向、真实 watcher、平台 parity、full_leaf、full_leaf_100 或 `voice` 接入声明。 |
...
| `docs/reconstruction/system-snapshot-bootstrap-current-source-map.md` | system snapshot-bootstrap 后端当前源码链路 | `scripts/validate-backend-system-owner.mjs` 直接验证本文、`load_snapshot`、`load_bootstrap_state`、Tauri 注册、command adapter、usecase owner、repository owner、Rust DTO、TypeScript service、IPC contract、E2E mock 和未声明边界；只把 `load_snapshot` 关闭到 snapshot-bootstrap 公开文件事实 owner，不声明 raw/internal gate、真实 daemon、watcher、autoswitch、maintenance/window 平台副作用、full_leaf、full_leaf_100 或 `voice` 接入。 |
| `docs/reconstruction/system-shell-init-duplicate-current-source-map.md` | system-shell-init 非权威 outtake | `scripts/validate-frontend-current-source-closeouts.mjs` 与 `scripts/validate-frontend-closeout-map-bindings.mjs` 直接验证本文、`duplicate_local_outtake_not_authoritative`、`authoritative_shared_bootstrap_work_exists`、非权威 closeout 边界和未声明字段；不作为 authoritative closeout，不关闭 raw/internal gate，不声明 readyToImplement、implementation_use、gate_accepted、full_leaf、full_leaf_100 或 `voice` 接入。 |
...
| `docs/reconstruction/maintenance-current-source-evidence-map.md` | maintenance 后端公开 owner 边界 | `scripts/validate-backend-maintenance-owner.mjs` 直接验证本文、maintenance command/usecase/repository、platform_actions 复用、system 回流禁止、Tauri 注册、source-map/README 索引和不声明事项；本文同时登记 `scripts/validate-backend-platform-actions-owner.mjs`、`scripts/validate-backend-diagnostics-owner.mjs`、`scripts/validate-backend-hexagonal.mjs` 与 `npm run validate:backend` 入口，不作为闭源 maintenance 引擎、真实更新重启闭环、平台动作真实副作用或 `voice` 接入声明。 |
...
| `docs/reconstruction/system-hotspot-current-source-map.md` | system hotspot 前端链路、macOS has_notch 只读探针与后端边界 | `scripts/validate-frontend-system-hotspot-current-source.mjs` 与 `scripts/validate-backend-hotspot-owner.mjs` 直接验证本文、前端 settings/system 链路、hotspot command/usecase/core/repository/platform owner、macOS `has_notch` 只读屏幕形态探针和未恢复边界。 |
...
| `docs/reconstruction/system-window-maintenance-current-source-map.md` | system-window-maintenance 前端当前源码链路 | `scripts/validate-frontend-system-window-maintenance-current-source.mjs` 与 `scripts/validate-frontend-current-source-closeouts.mjs` 直接验证本文、七条 non-gating closed command、`load_snapshot` snapshot-bootstrap owner、route/service/query/mutation/cache/mock/IPC contract 和 maintenance/window 平台副作用未声明边界。 |
...
| `docs/reconstruction/tray-current-source-evidence-map.md` | tray current-source owner 与公开文件事实读取边界 | `scripts/validate-backend-tray-owner.mjs` 与 `scripts/validate-frontend-tray-current-source.mjs` 直接验证本文、Windows accepted target、current-source extra、`tray_relay_usage_quota_model` 公开本地文件事实读取和 hidden shell 边界；不声明 native tray、运行时 relay、provider runtime、quota runtime、raw/internal gate 或双平台 full leaf 恢复。 |
...
| `docs/reconstruction/settings-api-proxy-current-source-map.md` | settings API proxy 当前源码链路 | `scripts/validate-frontend-settings-api-proxy-current-source.mjs` 与 `npm run validate:frontend-settings-api-proxy-current-source` 专名验证 settings API proxy 的 UI、controller、service、API、IPC、cache、mock 当前源码链路、mock env/probe payload 和后端受限平台端口边界；不声明 100% 还原、full leaf 完成、全功能完成、业务 HTTP 探测完成、系统代理扫描完成或 `voice` 接入。 |
