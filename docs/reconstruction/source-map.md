...
| `docs/reconstruction/mystery-unlock-current-source-map.md` | mystery unlock grants 与 route gate/helper | `scripts/validate-frontend-mystery-unlock-current-source.mjs` 与 `scripts/validate-frontend-mystery-gates.mjs` 直接验证本文、`get_mystery_unlock_grants`、`merge_mystery_unlock_grants` 的 system service、overview cache/query/mutation、mock、IPC contract、route gate/helper、closeout 台账和未声明边界；不登记 gate-report failure，不把 route helper 当 IPC command，不声明 dim6、gate_accepted、implementation_use、full_leaf_100 或 `voice` 接入。 |
