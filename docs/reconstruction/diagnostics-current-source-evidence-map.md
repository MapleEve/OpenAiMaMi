...
- `scripts/validate-backend-diagnostics-owner.mjs` 直接验证本文件、diagnostics usecase、repository、contracts、ports、TypeScript 类型、E2E mock、maintenance 转发、`catalog_integrity` 只读探针、TOML 语法/profile 冲突合同、平台私有字段不外泄和待处理/空操作边界。
