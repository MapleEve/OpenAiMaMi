...
| `src-tauri/src/application/usecase/diagnostics.rs` | diagnostics usecase 组装只读诊断 DTO、平台 os/arch、路径状态、registry/session 摘要、`catalog_integrity` 只读探针、file-shape 只读字段和 pending diagnostics 字段。 |
...
| `src-tauri/src/repository/diagnostics.rs` | diagnostics repository 只通过可替换 FS 读取路径存在性、registry JSON items 数量、sessions 子项数量、auth 文件大小、registry 文件形态和 sessions 目录形态。 |
| `src-tauri/src/repository/relay.rs` | `catalog_integrity` 只复用 relay repository 对 config.toml、`codex_router_catalog.json`、TOML 语法、profile 冲突和 relay 本地配置状态的只读事实，不执行修复。 |
| `src-tauri/src/contracts/diagnostics.rs` | 公开 `DiagnosePayload`、platform、snapshot、probe、`catalog_integrity` 和 pending field DTO。 |
| `src/types/index.ts` 与 `src/mocks/fixtures/commands.ts` | 仅作为 TypeScript 类型和 E2E mock 合同镜像，不证明运行时诊断能力恢复。 |
...
| sessions 数量 | `repository/diagnostics.rs` | 如果 sessions 目录存在，只统计目录子项数量；不读取敏感会话正文。 |
| file-shape 只读字段 | `repository/diagnostics.rs` 与 `application/usecase/diagnostics.rs` | `auth_file_shape` 只公开 auth 文件存在性和大小，不解析凭据内容；`registry_file_shape` 只公开 JSON 是否有效、items 数量、缺失 key 数量和重复 key 数量，不返回账号私密值；`session_rollout_file_shape` 只公开 sessions 目录条目数和 jsonl 文件数，不读取会话正文。 |
| `catalog_integrity` 只读探针 | `application/usecase/diagnostics.rs`、`repository/relay.rs` | 读取 config.toml 受管区块、`codex_router_catalog.json` 存在性、TOML 语法、profile 冲突、relay 本地 router 开关和 stale reason；不调用 router 修复，不写文件，不启动进程。 |
...
| pending 项 | 当前边界 |
| --- | --- |
| `auth_integrity` | 认证文件与平台密钥或注册表一致性诊断引擎未恢复；当前只新增 `auth_file_shape` 文件形态只读字段。 |
| `api_key_integrity` | API key 与平台凭据存储一致性诊断引擎未恢复。 |
| `db_orphan_providers` | SQLite 中转 Provider 孤儿记录诊断引擎未恢复。 |
| `rollout_orphan_providers` | rollout 线程孤儿 Provider 诊断引擎未恢复；当前只新增 `session_rollout_file_shape` 文件形态只读字段。 |
| `repair_logic` | 诊断修复逻辑未恢复；当前 `diagnose` 不写文件、不启动进程、不修改配置。 |
| 平台动作 | 注册表、钥匙串、系统修复、外部进程和平台原生诊断动作未恢复；当前只保留端口骨架、待处理和空操作边界。 |

## 验证入口

- `scripts/validate-backend-diagnostics-owner.mjs` 直接验证本文档、diagnostics usecase、repository、contracts、ports、TypeScript 类型、E2E mock、maintenance 转发、`catalog_integrity` 只读探针、file-shape 只读字段、TOML 语法/profile 冲突合同、平台私有字段不外泄和待处理/空操作边界。
