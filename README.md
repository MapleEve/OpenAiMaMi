...
| 后端 | 六边形目录、命令适配、用例层、核心层、仓储层、平台层、适配器、合同、DTO、公开文件事实聚合、账号监视仓储预检、通知客户端状态 settings 持久化、会话仓储写入状态与 effect/status 边界已进入聚合验证。 | 不做闭源业务全量还原；无公开证据支撑的业务保持类型化待实现、空操作或可替换骨架边界；不声明动态插件执行、真实平台副作用、外部进程、市场安装、真实网络重试、完整服务启动或 `voice` 集成已经完成。 | `npm run validate:backend` |
