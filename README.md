...
| 后端 | `src-tauri/` 已建立 commands、application、core、platform、repository、adapters、contracts 六边形骨架，并补回已有公开证据支撑的后端能力；maintenance 文件事务已收口到 repository owner。 | 不做闭源后端业务全量还原；未公开、未选择还原或无公开证据支撑的行为只能保留为契约、桩、待实现项、测试缺口或待补证据位置。 |
