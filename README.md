...
- 新增 `scripts/validate-frontend-leaf-copy-acceptance.mjs`，把前端 leaf 和全文案验收从静态覆盖扫描中分离出来；全文案验收清单 `evidence/full-chain/internal/frontend-copy-acceptance.json` 已同步当前 locale，现为 `status=draft`，记录 `entries=660`、`acceptedZh=613`、`acceptedEn=613`、`missingRawOrInternalCopySource=47`，说明新增 time/router/templates/mock payload 文案仍需补 raw/internal 文案来源后才能关闭全文案验收。
- `README.md` 与 `README-cn.md` 保持中文同步，记录当前已经做了什么、没有做什么；本轮进度服务于 main 直接开发，每次提交都应同步更新 README，避免进度说明再次落后于证据和代码状态。

### 未做

- 前端 macOS / Windows 双平台 100% leaf 尚未完成验收；现有 `validate-frontend-dumped`、`validate-frontend-evidence` 和 `validate-i18n` 只能证明静态覆盖、owner 边界和 locale key 同步；全文案清单当前是 draft，新增 47 个 time/router/templates/mock payload 文案缺 raw/internal 来源，不能证明所有 leaf、渲染、交互和文案语义已逐条还原。
