...
| 后端 | 已按命令适配、用例层、核心层、仓储层、平台层、适配器、合同和 DTO 组织六边形公开骨架；公开能力归属统一由后端 owner 验证脚本约束。 | 不补写无公开证据支撑的闭源业务逻辑；相关位置只保留接口、DTO、错误语义、测试占位、待补证据或可替换骨架。 | `npm run validate:backend`、`npm run validate:backend-cargo` |
