...
OpenAiMami 是面向个人本地工作流的桌面应用公开重建仓库。本仓库不记录提交流水账，而是把匿名化证据、可审计架构骨架、已开放能力和验证口径放在同一条链路中，支持个人长期迭代，也让使用者和贡献者用得更放心，降低隐私泄露、越界还原和无证据实现的风险。
...
| 路径或材料 | 说明 |
| --- | --- |
| `docs/reconstruction/` | 重建规则、原文决策、证据入口、发布规则、架构说明、索引和大文件策略。 |
| `evidence/full-chain/raw/` | 匿名化 raw 主链路，作为继续还原前的主入口。 |
| `evidence/full-chain/internal/` | 匿名化 internal 整理链路，承载审计映射、前端映射、蒸馏逻辑和索引。 |
| `evidence/binary-manifests/` | 外部大文件的公开状态、大小和哈希清单。 |
| `src/` | 当前公开前端源代码和按归属边界整理后的模块化骨架。 |
...
先校验 evidence/full-chain/raw 和 evidence/full-chain/internal：读取索引、manifest、校验摘要、前端 dumped 材料、IPC、CCF、audit map、frontend map、distilled logic 和 raw leaf，并确认后续实现都能回指公开证据。

前端以全量还原为目标，按公开证据补齐入口、路由、运行时、模块 Provider、StoreUpdater、Content、cache、hooks、dialogs、panels、components、types、测试和文档。不要声明已经完成双平台百分百全量还原，除非公开证据、叶子队列、收口记录和验证脚本都能支撑。
...
- 前端 PR 应说明使用了哪些 `raw/internal` 证据，以及补齐了哪些入口、路由、运行时、服务门面、缓存、弹窗、面板、组件、类型和测试。
- 后端 PR 应说明补齐了哪些 command、usecase、core、repository、platform、adapter、contract 和 DTO；只能补入开源已有能力或公开证据支撑的能力。
- 无公开证据支撑的行为不能写成真实业务逻辑，只能写成合同、骨架、待实现项、测试缺口或可替换接口。
...
- 内部项目名。
- 设备标识、个人目录、共享路径或绝对本地路径。
- 凭据、令牌、会话、密钥、账号私密值或未脱敏日志。
- 个人数据、客户数据、运行时缓存或未审查采集材料。
- 外部参考仓库名。
